...
Runs recorded: 78 · receipts: 834 (MEASURED from files present; cron is best-effort, gaps are honest)

Track record (latest, MEASURED from verified receipts only): +1d n=0 hit=— pending=0 · +7d n=0 hit=— pending=0 · no-calls(BLOCKED)=289 — a past frequency, NOT a prediction

Hash chain: 76 link(s), head seq 76 sha256 d6b45dedc2b3… — every sealed run tamper-evident; walk it: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --chain ledger/`

Paper book (MODELED, paper-only — NOT real funds): seq 70 · equity $10000.000000 · open positions 0 · fills this run 0 · replay it: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --book ledger/`

Refusal record (MEASURED): latest run BLOCKED 3/6 — liquidity×2 conviction×1 · lifetime (recorded runs) 254/414 — liquidity×140 conviction×123 · a refusal is a decision, not an absence · replay: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --refusals ledger/`

External witness (REPORTED, SET + Merkle inclusion + log consistency offline-verifiable): chain head seq 76 anchored in Rekor — logIndex 2310438172, uuid 108e9186e8c5677a… · heads anchored 76/76 · inclusion proven offline 76/76 · log consistency 75/75 adjacent checkpoint pair(s) receipted · second witness (RFC 3161) 76/76 head(s) countersigned · cross-witness gossip 63 observation(s) from a second scheduled observer · an anchored head cannot be silently truncated · check: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --witness .`
