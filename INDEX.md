...
Runs recorded: 61 · receipts: 647 (MEASURED from files present; cron is best-effort, gaps are honest)

Track record (latest, MEASURED from verified receipts only): +1d n=0 hit=— pending=0 · +7d n=0 hit=— pending=0 · no-calls(BLOCKED)=237 — a past frequency, NOT a prediction

Hash chain: 59 link(s), head seq 59 sha256 59af86b668a4… — every sealed run tamper-evident; walk it: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --chain ledger/`

Paper book (MODELED, paper-only — NOT real funds): seq 53 · equity $10000.000000 · open positions 0 · fills this run 0 · replay it: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --book ledger/`

Refusal record (MEASURED): latest run BLOCKED 3/6 — liquidity×2 conviction×1 · lifetime (recorded runs) 202/312 — conviction×105 liquidity×105 · a refusal is a decision, not an absence · replay: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --refusals ledger/`

External witness (REPORTED, SET + Merkle inclusion + log consistency offline-verifiable): chain head seq 59 anchored in Rekor — logIndex 2265834478, uuid 108e9186e8c5677a… · heads anchored 59/59 · inclusion proven offline 59/59 · log consistency 58/58 adjacent checkpoint pair(s) receipted · second witness (RFC 3161) 59/59 head(s) countersigned · cross-witness gossip 46 observation(s) from a second scheduled observer · an anchored head cannot be silently truncated · check: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --witness .`
