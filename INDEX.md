...
Runs recorded: 95 · receipts: 1021 (MEASURED from files present; cron is best-effort, gaps are honest)

Track record (latest, MEASURED from verified receipts only): +1d n=0 hit=— pending=0 · +7d n=0 hit=— pending=0 · no-calls(BLOCKED)=327 — a past frequency, NOT a prediction

Hash chain: 93 link(s), head seq 93 sha256 97cb0719a5a4… — every sealed run tamper-evident; walk it: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --chain ledger/`

Paper book (MODELED, paper-only — NOT real funds): seq 87 · equity $10000.000000 · open positions 0 · fills this run 0 · replay it: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --book ledger/`

Refusal record (MEASURED): latest run BLOCKED 2/6 — liquidity×2 · lifetime (recorded runs) 292/516 — liquidity×178 conviction×125 · a refusal is a decision, not an absence · replay: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --refusals ledger/`

External witness (REPORTED, SET + Merkle inclusion + log consistency offline-verifiable): chain head seq 93 anchored in Rekor — logIndex 2345559989, uuid 108e9186e8c5677a… · heads anchored 93/93 · inclusion proven offline 93/93 · log consistency 92/92 adjacent checkpoint pair(s) receipted · second witness (RFC 3161) 93/93 head(s) countersigned · cross-witness gossip 80 observation(s) from a second scheduled observer · an anchored head cannot be silently truncated · check: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --witness .`
