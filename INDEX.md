...
Runs recorded: 58 · receipts: 614 (MEASURED from files present; cron is best-effort, gaps are honest)

Track record (latest, MEASURED from verified receipts only): +1d n=0 hit=— pending=0 · +7d n=0 hit=— pending=0 · no-calls(BLOCKED)=226 — a past frequency, NOT a prediction

Hash chain: 56 link(s), head seq 56 sha256 4d1b7dbc9eea… — every sealed run tamper-evident; walk it: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --chain ledger/`

Paper book (MODELED, paper-only — NOT real funds): seq 50 · equity $10000.000000 · open positions 0 · fills this run 0 · replay it: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --book ledger/`

Refusal record (MEASURED): latest run BLOCKED 4/6 — conviction×2 liquidity×2 · lifetime (recorded runs) 191/294 — conviction×100 liquidity×99 · a refusal is a decision, not an absence · replay: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --refusals ledger/`

External witness (REPORTED, SET + Merkle inclusion + log consistency offline-verifiable): chain head seq 56 anchored in Rekor — logIndex 2257243143, uuid 108e9186e8c5677a… · heads anchored 56/56 · inclusion proven offline 56/56 · log consistency 55/55 adjacent checkpoint pair(s) receipted · second witness (RFC 3161) 56/56 head(s) countersigned · cross-witness gossip 43 observation(s) from a second scheduled observer · an anchored head cannot be silently truncated · check: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --witness .`
