...
Runs recorded: 73 · receipts: 779 (MEASURED from files present; cron is best-effort, gaps are honest)

Track record (latest, MEASURED from verified receipts only): +1d n=0 hit=— pending=0 · +7d n=0 hit=— pending=0 · no-calls(BLOCKED)=274 — a past frequency, NOT a prediction

Hash chain: 71 link(s), head seq 71 sha256 3591dbf1466a… — every sealed run tamper-evident; walk it: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --chain ledger/`

Paper book (MODELED, paper-only — NOT real funds): seq 65 · equity $10000.000000 · open positions 0 · fills this run 0 · replay it: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --book ledger/`

Refusal record (MEASURED): latest run BLOCKED 3/6 — liquidity×2 conviction×1 · lifetime (recorded runs) 239/384 — liquidity×129 conviction×118 · a refusal is a decision, not an absence · replay: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --refusals ledger/`

External witness (REPORTED, SET + Merkle inclusion + log consistency offline-verifiable): chain head seq 71 anchored in Rekor — logIndex 2297026036, uuid 108e9186e8c5677a… · heads anchored 71/71 · inclusion proven offline 71/71 · log consistency 70/70 adjacent checkpoint pair(s) receipted · second witness (RFC 3161) 71/71 head(s) countersigned · cross-witness gossip 58 observation(s) from a second scheduled observer · an anchored head cannot be silently truncated · check: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --witness .`
