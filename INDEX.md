...
Runs recorded: 98 · receipts: 1054 (MEASURED from files present; cron is best-effort, gaps are honest)

Track record (latest, MEASURED from verified receipts only): +1d n=0 hit=— pending=0 · +7d n=0 hit=— pending=0 · no-calls(BLOCKED)=335 — a past frequency, NOT a prediction

Hash chain: 96 link(s), head seq 96 sha256 17681fc254f9… — every sealed run tamper-evident; walk it: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --chain ledger/`

Paper book (MODELED, paper-only — NOT real funds): seq 90 · equity $10000.000000 · open positions 0 · fills this run 0 · replay it: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --book ledger/`

Refusal record (MEASURED): latest run BLOCKED 3/6 — liquidity×3 · lifetime (recorded runs) 300/534 — liquidity×186 conviction×125 · a refusal is a decision, not an absence · replay: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --refusals ledger/`

External witness (REPORTED, SET + Merkle inclusion + log consistency offline-verifiable): chain head seq 96 anchored in Rekor — logIndex 2358087965, uuid 108e9186e8c5677a… · heads anchored 96/96 · inclusion proven offline 96/96 · log consistency 95/95 adjacent checkpoint pair(s) receipted · second witness (RFC 3161) 96/96 head(s) countersigned · cross-witness gossip 83 observation(s) from a second scheduled observer · an anchored head cannot be silently truncated · check: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --witness .`
