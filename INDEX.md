...
Runs recorded: 64 · receipts: 680 (MEASURED from files present; cron is best-effort, gaps are honest)

Track record (latest, MEASURED from verified receipts only): +1d n=0 hit=— pending=0 · +7d n=0 hit=— pending=0 · no-calls(BLOCKED)=247 — a past frequency, NOT a prediction

Hash chain: 62 link(s), head seq 62 sha256 17481cd4b5ea… — every sealed run tamper-evident; walk it: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --chain ledger/`

Paper book (MODELED, paper-only — NOT real funds): seq 56 · equity $10000.000000 · open positions 0 · fills this run 0 · replay it: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --book ledger/`

Refusal record (MEASURED): latest run BLOCKED 4/6 — conviction×2 liquidity×2 · lifetime (recorded runs) 212/330 — liquidity×111 conviction×109 · a refusal is a decision, not an absence · replay: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --refusals ledger/`

External witness (REPORTED, SET + Merkle inclusion + log consistency offline-verifiable): chain head seq 62 anchored in Rekor — logIndex 2276012169, uuid 108e9186e8c5677a… · heads anchored 62/62 · inclusion proven offline 62/62 · log consistency 61/61 adjacent checkpoint pair(s) receipted · second witness (RFC 3161) 62/62 head(s) countersigned · cross-witness gossip 49 observation(s) from a second scheduled observer · an anchored head cannot be silently truncated · check: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --witness .`
