...
Runs recorded: 59 · receipts: 625 (MEASURED from files present; cron is best-effort, gaps are honest)

Track record (latest, MEASURED from verified receipts only): +1d n=0 hit=— pending=0 · +7d n=0 hit=— pending=0 · no-calls(BLOCKED)=230 — a past frequency, NOT a prediction

Hash chain: 57 link(s), head seq 57 sha256 20c0458b8941… — every sealed run tamper-evident; walk it: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --chain ledger/`

Paper book (MODELED, paper-only — NOT real funds): seq 51 · equity $10000.000000 · open positions 0 · fills this run 0 · replay it: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --book ledger/`

Refusal record (MEASURED): latest run BLOCKED 4/6 — conviction×2 liquidity×2 · lifetime (recorded runs) 195/300 — conviction×102 liquidity×101 · a refusal is a decision, not an absence · replay: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --refusals ledger/`

External witness (REPORTED, SET + Merkle inclusion + log consistency offline-verifiable): chain head seq 57 anchored in Rekor — logIndex 2258325806, uuid 108e9186e8c5677a… · heads anchored 57/57 · inclusion proven offline 57/57 · log consistency 56/56 adjacent checkpoint pair(s) receipted · second witness (RFC 3161) 57/57 head(s) countersigned · cross-witness gossip 44 observation(s) from a second scheduled observer · an anchored head cannot be silently truncated · check: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --witness .`
