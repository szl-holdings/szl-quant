...
Runs recorded: 57 · receipts: 603 (MEASURED from files present; cron is best-effort, gaps are honest)

Track record (latest, MEASURED from verified receipts only): +1d n=0 hit=— pending=0 · +7d n=0 hit=— pending=0 · no-calls(BLOCKED)=222 — a past frequency, NOT a prediction

Hash chain: 55 link(s), head seq 55 sha256 7975f1f33f1b… — every sealed run tamper-evident; walk it: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --chain ledger/`

Paper book (MODELED, paper-only — NOT real funds): seq 49 · equity $10000.000000 · open positions 0 · fills this run 0 · replay it: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --book ledger/`

Refusal record (MEASURED): latest run BLOCKED 4/6 — conviction×2 liquidity×2 · lifetime (recorded runs) 187/288 — conviction×98 liquidity×97 · a refusal is a decision, not an absence · replay: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --refusals ledger/`

External witness (REPORTED, SET + Merkle inclusion + log consistency offline-verifiable): chain head seq 55 anchored in Rekor — logIndex 2256863947, uuid 108e9186e8c5677a… · heads anchored 55/55 · inclusion proven offline 55/55 · log consistency 54/54 adjacent checkpoint pair(s) receipted · second witness (RFC 3161) 55/55 head(s) countersigned · cross-witness gossip 42 observation(s) from a second scheduled observer · an anchored head cannot be silently truncated · check: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --witness .`
