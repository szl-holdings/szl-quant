...
Runs recorded: 70 · receipts: 746 (MEASURED from files present; cron is best-effort, gaps are honest)

Track record (latest, MEASURED from verified receipts only): +1d n=0 hit=— pending=0 · +7d n=0 hit=— pending=0 · no-calls(BLOCKED)=265 — a past frequency, NOT a prediction

Hash chain: 68 link(s), head seq 68 sha256 56e68ff67b25… — every sealed run tamper-evident; walk it: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --chain ledger/`

Paper book (MODELED, paper-only — NOT real funds): seq 62 · equity $10000.000000 · open positions 0 · fills this run 0 · replay it: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --book ledger/`

Refusal record (MEASURED): latest run BLOCKED 3/6 — liquidity×2 conviction×1 · lifetime (recorded runs) 230/366 — liquidity×123 conviction×115 · a refusal is a decision, not an absence · replay: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --refusals ledger/`

External witness (REPORTED, SET + Merkle inclusion + log consistency offline-verifiable): chain head seq 68 anchored in Rekor — logIndex 2287972887, uuid 108e9186e8c5677a… · heads anchored 68/68 · inclusion proven offline 68/68 · log consistency 67/67 adjacent checkpoint pair(s) receipted · second witness (RFC 3161) 68/68 head(s) countersigned · cross-witness gossip 55 observation(s) from a second scheduled observer · an anchored head cannot be silently truncated · check: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --witness .`
