...
Runs recorded: 60 · receipts: 636 (MEASURED from files present; cron is best-effort, gaps are honest)

Track record (latest, MEASURED from verified receipts only): +1d n=0 hit=— pending=0 · +7d n=0 hit=— pending=0 · no-calls(BLOCKED)=234 — a past frequency, NOT a prediction

Hash chain: 58 link(s), head seq 58 sha256 882706b4d437… — every sealed run tamper-evident; walk it: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --chain ledger/`

Paper book (MODELED, paper-only — NOT real funds): seq 52 · equity $10000.000000 · open positions 0 · fills this run 0 · replay it: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --book ledger/`

Refusal record (MEASURED): latest run BLOCKED 4/6 — conviction×2 liquidity×2 · lifetime (recorded runs) 199/306 — conviction×104 liquidity×103 · a refusal is a decision, not an absence · replay: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --refusals ledger/`

External witness (REPORTED, SET + Merkle inclusion + log consistency offline-verifiable): chain head seq 58 anchored in Rekor — logIndex 2260796141, uuid 108e9186e8c5677a… · heads anchored 58/58 · inclusion proven offline 58/58 · log consistency 57/57 adjacent checkpoint pair(s) receipted · second witness (RFC 3161) 58/58 head(s) countersigned · cross-witness gossip 45 observation(s) from a second scheduled observer · an anchored head cannot be silently truncated · check: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --witness .`
