...
Runs recorded: 69 · receipts: 735 (MEASURED from files present; cron is best-effort, gaps are honest)

Track record (latest, MEASURED from verified receipts only): +1d n=0 hit=— pending=0 · +7d n=0 hit=— pending=0 · no-calls(BLOCKED)=262 — a past frequency, NOT a prediction

Hash chain: 67 link(s), head seq 67 sha256 a3e0ddc8300d… — every sealed run tamper-evident; walk it: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --chain ledger/`

Paper book (MODELED, paper-only — NOT real funds): seq 61 · equity $10000.000000 · open positions 0 · fills this run 0 · replay it: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --book ledger/`

Refusal record (MEASURED): latest run BLOCKED 3/6 — liquidity×2 conviction×1 · lifetime (recorded runs) 227/360 — liquidity×121 conviction×114 · a refusal is a decision, not an absence · replay: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --refusals ledger/`

External witness (REPORTED, SET + Merkle inclusion + log consistency offline-verifiable): chain head seq 67 anchored in Rekor — logIndex 2285553958, uuid 108e9186e8c5677a… · heads anchored 67/67 · inclusion proven offline 67/67 · log consistency 66/66 adjacent checkpoint pair(s) receipted · second witness (RFC 3161) 67/67 head(s) countersigned · cross-witness gossip 54 observation(s) from a second scheduled observer · an anchored head cannot be silently truncated · check: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --witness .`
