...
Runs recorded: 47 · receipts: 493 (MEASURED from files present; cron is best-effort, gaps are honest)

Track record (latest, MEASURED from verified receipts only): +1d n=0 hit=— pending=0 · +7d n=0 hit=— pending=0 · no-calls(BLOCKED)=182 — a past frequency, NOT a prediction

Hash chain: 45 link(s), head seq 45 sha256 b88e16c2c531… — every sealed run tamper-evident; walk it: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --chain ledger/`

Paper book (MODELED, paper-only — NOT real funds): seq 39 · equity $10000.000000 · open positions 0 · fills this run 0 · replay it: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --book ledger/`

Refusal record (MEASURED): latest run BLOCKED 4/6 — conviction×2 liquidity×2 · lifetime (recorded runs) 147/228 — conviction×78 liquidity×77 · a refusal is a decision, not an absence · replay: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --refusals ledger/`

External witness (REPORTED, SET + Merkle inclusion + log consistency offline-verifiable): chain head seq 45 anchored in Rekor — logIndex 2234791740, uuid 108e9186e8c5677a… · heads anchored 45/45 · inclusion proven offline 45/45 · log consistency 44/44 adjacent checkpoint pair(s) receipted · second witness (RFC 3161) 45/45 head(s) countersigned · cross-witness gossip 32 observation(s) from a second scheduled observer · an anchored head cannot be silently truncated · check: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --witness .`
