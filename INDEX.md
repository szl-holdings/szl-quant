...
Runs recorded: 102 · receipts: 1098 (MEASURED from files present; cron is best-effort, gaps are honest)

Track record (latest, MEASURED from verified receipts only): +1d n=0 hit=— pending=0 · +7d n=0 hit=— pending=0 · no-calls(BLOCKED)=344 — a past frequency, NOT a prediction

Hash chain: 100 link(s), head seq 100 sha256 c0a882052364… — every sealed run tamper-evident; walk it: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --chain ledger/`

Paper book (MODELED, paper-only — NOT real funds): seq 94 · equity $10000.000000 · open positions 0 · fills this run 0 · replay it: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --book ledger/`

Refusal record (MEASURED): latest run BLOCKED 3/6 — liquidity×3 · lifetime (recorded runs) 309/558 — liquidity×195 conviction×125 · a refusal is a decision, not an absence · replay: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --refusals ledger/`

External witness (REPORTED, SET + Merkle inclusion + log consistency offline-verifiable): chain head seq 100 anchored in Rekor — logIndex 2370479354, uuid 108e9186e8c5677a… · heads anchored 100/100 · inclusion proven offline 100/100 · log consistency 99/99 adjacent checkpoint pair(s) receipted · second witness (RFC 3161) 100/100 head(s) countersigned · cross-witness gossip 87 observation(s) from a second scheduled observer · an anchored head cannot be silently truncated · check: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --witness .`
