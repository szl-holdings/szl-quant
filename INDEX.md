...
Runs recorded: 55 · receipts: 581 (MEASURED from files present; cron is best-effort, gaps are honest)

Track record (latest, MEASURED from verified receipts only): +1d n=0 hit=— pending=0 · +7d n=0 hit=— pending=0 · no-calls(BLOCKED)=214 — a past frequency, NOT a prediction

Hash chain: 53 link(s), head seq 53 sha256 04d0794e5ae8… — every sealed run tamper-evident; walk it: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --chain ledger/`

Paper book (MODELED, paper-only — NOT real funds): seq 47 · equity $10000.000000 · open positions 0 · fills this run 0 · replay it: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --book ledger/`

Refusal record (MEASURED): latest run BLOCKED 4/6 — conviction×2 liquidity×2 · lifetime (recorded runs) 179/276 — conviction×94 liquidity×93 · a refusal is a decision, not an absence · replay: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --refusals ledger/`

External witness (REPORTED, SET + Merkle inclusion + log consistency offline-verifiable): chain head seq 53 anchored in Rekor — logIndex 2256178530, uuid 108e9186e8c5677a… · heads anchored 53/53 · inclusion proven offline 53/53 · log consistency 52/52 adjacent checkpoint pair(s) receipted · second witness (RFC 3161) 53/53 head(s) countersigned · cross-witness gossip 40 observation(s) from a second scheduled observer · an anchored head cannot be silently truncated · check: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --witness .`
