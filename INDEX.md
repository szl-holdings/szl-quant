...
Runs recorded: 45 · receipts: 471 (MEASURED from files present; cron is best-effort, gaps are honest)

Track record (latest, MEASURED from verified receipts only): +1d n=0 hit=— pending=0 · +7d n=0 hit=— pending=0 · no-calls(BLOCKED)=174 — a past frequency, NOT a prediction

Hash chain: 43 link(s), head seq 43 sha256 9c0e7d545b9f… — every sealed run tamper-evident; walk it: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --chain ledger/`

Paper book (MODELED, paper-only — NOT real funds): seq 37 · equity $10000.000000 · open positions 0 · fills this run 0 · replay it: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --book ledger/`

Refusal record (MEASURED): latest run BLOCKED 4/6 — conviction×2 liquidity×2 · lifetime (recorded runs) 139/216 — conviction×74 liquidity×73 · a refusal is a decision, not an absence · replay: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --refusals ledger/`

External witness (REPORTED, SET + Merkle inclusion + log consistency offline-verifiable): chain head seq 43 anchored in Rekor — logIndex 2230843526, uuid 108e9186e8c5677a… · heads anchored 43/43 · inclusion proven offline 43/43 · log consistency 42/42 adjacent checkpoint pair(s) receipted · second witness (RFC 3161) 43/43 head(s) countersigned · cross-witness gossip 30 observation(s) from a second scheduled observer · an anchored head cannot be silently truncated · check: `node verify/verify.mjs --pubkey keys/engine_pubkey.json --witness .`
